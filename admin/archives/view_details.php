...
<div class="container-fluid">
    <div class="card card-outline card-navy rounded-0 shadow">
        <div class="card-header rounded-0">
            <h5 class="card-title">Détails de l'inscrit</h5>
        </div>
        <div class="card-body rounded-0">
            ...
            <div class="row">
                <div class="col-md-6">
                    <dl>
                        ...
                        <dt class="text-muted">Date d'anniversaire</dt>
                        ...
                        <dt class="text-muted">Adresse email</dt>
                ...
                <div class="col-md-6">
                    <dl>
                        <dt class="text-muted">Bagage de formation</dt>
            ...
            <?php if(in_array($status,range(1,3))): ?>
                <fieldset>
                    <legend class="text-navy">Historique de paiement</legend>
                    ...
                    <table class="table table-hover table-striped table-bordered" id="payment-list">
                        ...
                        <thead>
                            <tr>
                                <th class="text-center py-1">DateHeure</th>
                                <th class="text-center py-1">Remarques</th>
                                <th class="text-center py-1">Montant payé</th>
                                <th class="text-center py-1">Solde d'argent</th>
        ...
        <div class="card-footer rounded-0 text-center">
                <button class="btn btn-sm btn-primary btn-flat" type="button" id="update_status"><i class="fa fa-save"></i> Mettre à jour le statut</button>
                <button class="btn btn-sm btn-light border btn-flat" type="button" id="payment"><i class="fa fa-plus"></i> Paiement</button>
                <button class="btn btn-sm btn-success btn-flat" type="button" id="print"><i class="fa fa-print"></i> Imprimer</button>
        </div>
    </div>
...
<script>
    $(function(){
        $('#update_status').click(function(){
            uni_modal("Modifier le statut","enrollees/update_status.php?id=<?= $id ?>&status=<?= $status ?>")
        })
        $('#payment').click(function(){
            uni_modal("New Payment","enrollees/payment.php?id=<?= $id ?>&balance=<?= $balance ?>")
        })
        $('.delete_payment').click(function(){
			_conf("Êtes-vous sûr de supprimer définitivement ce paiement ?","delete_payment",[$(this).attr('data-id')])
		})
        $('#print').click(function(){
            ...
            _p.prepend("<div class='mx-5 py-4'><h1 class='text-center'><?= $_settings->info("name") ?></h1>"
                        +"<h5 class='text-center'>Détails de l'enrôlé</h5></div>")
