...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT c.*, d.name as department from `tbl_filieres` c inner join `department_list` d on c.department_id = d.id where c.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <dl>
        <dt class="text-muted">Département</dt>
