...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		foreach($_POST as $k => $v){
			if(in_array($k,array('email_admin','firstname','middlename','lastname','username','type'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Détails de l\'utilisateur enregistrés avec succès.');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
			...
			if($qry){
				$this->settings->set_flashdata('success','Détails de l\'utilisateur mis à jour avec succès.');
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(!in_array($type,$allowed)){
				$resp['msg'].=" Mais l'image n'a pas pu être téléchargée en raison d'un type de fichier non valide,format .png et .jpeg seulement autorisé.";
			}else{
				$new_height = 200; 
				$new_width = 200; 
	...
	public function delete_users(){
		...
		if($qry){
			...
			$this->settings->set_flashdata('success','Détails de l\'utilisateur supprimés avec succès.');
	...
	public function save_student(){
		...
		if(empty($id)){
			...
			if($qry){
				$id = $this->conn->insert_id;
				$this->settings->set_flashdata('success','Les détails de l\'utilisateur étudiant ont été enregistrés avec succès.');
				$resp['status'] = "success";
			}else{
				$resp['status'] = "failed";
				$resp['msg'] = "Une erreur s'est produite lors de l'enregistrement des données. Error: ". $this->conn->error;
			}

		}else{
			$qry = $this->conn->query("UPDATE student_list set $data where id = {$id}");
			if($qry){
				$this->settings->set_flashdata('success','Les détails de l\'utilisateur étudiant ont été mis à jour avec succès.');
				if($id == $this->settings->userdata('id')){
					foreach($_POST as $k => $v){
						if($k != 'id'){
							if(!empty($data)) $data .=" , ";
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			$allowed = array('image/png','image/jpeg');
			if(!in_array($type,$allowed)){
				$resp['msg'].=" Mais l'image n'a pas pu être téléchargée en raison d'un type de fichier non valide,format .png et .jpeg seulement acceptable.";
			}else{
				$new_height = 200; 
				$new_width = 200; 
	...
	public function verify_student(){
		...
		if($update){
			$this->settings->set_flashdata('success','Le compte étudiant a été vérifié avec succès.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
		}
